    ...
    <div className="py-16 px-4 md:px-8">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500 mt-2">
          Stay updated with trending stories and major events
        </p>
      </div>
      ...
      >
        {news.map((item, index) => (
          <SwiperSlide key={index}>
            <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-8 bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
              {/* Text Content */}
              <div className="flex-1">
                <Link to="/">
                  <h3 className="text-xl font-semibold text-gray-800 hover:text-blue-500 transition-colors duration-200 mb-4">
                    {item.title}
                  </h3>
                </Link>
                <div className="w-16 h-[4px] bg-primary mb-5 rounded-full"></div>
              ...
              {/* Image */}
              <div className="flex-shrink-0 w-full sm:w-48 overflow-hidden rounded-lg shadow-md">
                <img
                  src={item.image}
                  alt={item.title}
